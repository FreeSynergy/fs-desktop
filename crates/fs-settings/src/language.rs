...
async fn install_language_pack(locale: LocaleInfo) -> Result<(), String> {
    ...
    let file_path = match StoreClient::node_store().fetch_raw(&url).await {
        Ok(content) => {
            let dest_dir = fs_dir.join("i18n").join(&locale.code);
    ...
    rsx! {
        div {
            style: "display: flex; border-bottom: 1px solid var(--fs-color-border-default); \
                    ...
                    background: var(--fs-color-bg-surface);",
            TabBtn {
                label:     fs_i18n::t("settings.language.tabs.active"),
                is_active: active == LangTab::Active,
                onclick:   move |_| on_select.call(LangTab::Active),
            }
            TabBtn {
                label:     fs_i18n::t("settings.language.tabs.install"),
            ...
            if !builtin {
                button {
                    style: "margin-left: 8px; padding: 2px 8px; font-size: 11px; \
                            ...
                            border-radius: var(--fs-radius-sm); cursor: pointer; opacity: 0.6;",
