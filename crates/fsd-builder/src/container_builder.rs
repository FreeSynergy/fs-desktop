...
pub fn ContainerBuilder() -> Element {
    ...
    rsx! {
        div {
            class: "fsd-container-app-builder",
            style: "padding: 24px;",
            ...
            match *step.read() {
                ...
                BuilderStep::Input => rsx! {
                    ...
                    if let Some(err) = parse_error.read().as_deref() {
                        div {
                            style: "color: var(--fsn-color-error); font-size: 13px; margin-bottom: 8px;",
                            "{err}"
                        }
                    }
                                ...
                                div {
                                    ...
                                    div {
                                        style: "display: grid; grid-template-columns: 2fr 1fr 1fr auto; \
                                                padding: 6px 12px; background: var(--fsn-color-bg-surface); \
                        ...
                        div {
                            ...
                            div { style: "display: flex; gap: 8px; margin-bottom: 8px;",
                                input {
                                    ...
                                    style: "flex: 1; padding: 6px 10px; border: 1px solid var(--fsn-color-border-default); \
                                            border-radius: var(--fsn-radius-md); font-size: 13px;",
                                    oninput: move |e| *ai_prompt.write() = e.value(),
                                }
                                button {
                                    style: "padding: 6px 16px; background: var(--fsn-color-primary); color: white; \
                                            border: none; border-radius: var(--fsn-radius-md); cursor: pointer; \
                ...
                BuilderStep::Export => rsx! {
                    ...
                    div { style: "margin-bottom: 16px;",
                        ...
                        input {
                            ...
                            style: "width: 100%; padding: 8px 12px; border: 1px solid var(--fsn-color-border-default); \
                                    border-radius: var(--fsn-radius-md); font-family: var(--fsn-font-mono); font-size: 13px; \
                    ...
                    div { style: "display: flex; gap: 8px;",
                        ...
                        button {
                            style: "padding: 8px 24px; background: var(--fsn-color-bg-surface); color: var(--fsn-color-text-primary); \
                                    border: 1px solid var(--fsn-color-border-default); \
                                    border-radius: var(--fsn-radius-md); cursor: pointer;",
                            onclick: move |_| {
                        ...
                        button {
                            ...
                            onclick: move |_| {
                                *export_msg.write() = Some(
                                    "Use `fsn-builder publish <dir>` to sign and publish to the Store.".into()
                                );
                            },
                            "Publish to Store"
                        }
...
fn VariableRow(
    ...
    rsx! {
        div {
            style: "display: grid; grid-template-columns: 2fr 1fr 1fr auto; \
                    padding: 6px 12px; border-top: 1px solid var(--fsn-color-border-default); \
                    font-size: 12px; align-items: center; {bg}",
            span {
                style: "font-family: var(--fsn-font-mono); font-size: 11px; \
                        color: var(--fsn-color-text-primary); overflow: hidden; text-overflow: ellipsis;",
                title: "{description}",
                "{name}"
            }
            span {
            ...
            span {
                style: if !role_label.is_empty() {
                    "color: var(--fsn-color-primary); font-weight: 500;"
                } else {
...
fn build_resource(yaml: &str, compose: ComposeFile) -> Result<ContainerResource, String> {
    ...
    use fsn_types::resources::validator::Validate;
    resource.validate();
...
fn detect_roles(image: &str) -> Vec<fsn_types::resources::meta::Role> {
