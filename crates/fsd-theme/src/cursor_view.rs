...
fn CursorCard(
    ...
    let border = if is_selected {
        "border: 2px solid var(--fsn-color-primary, #00bcd4);"
    } else {
        "border: 2px solid var(--fsn-color-border-default, #334155);"
    };

    rsx! {
        div {
            style: "display: flex; align-items: center; gap: 16px; \
                    padding: 14px 16px; border-radius: var(--fsn-radius-md, 8px); \
            ...
            div { style: "flex: 1;",
                div {
                    style: "font-size: 14px; font-weight: 600; \
                            color: var(--fsn-color-text-primary); margin-bottom: 2px;",
                    "{option.name}"
                }
                div {
            ...
            if is_selected {
                span {
                    style: "font-size: 16px; color: var(--fsn-color-primary, #00bcd4);",
                    "✓"
                }
            }
        }
