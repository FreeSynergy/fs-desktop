// `fs-store.db` — Store-specific storage.
//
...
/// Database handle for `fs-store.db`.
pub struct StoreDb {
    conn: DbConnection,
}

impl StoreDb {
    /// Open (or create) `~/.local/share/fsn/fs-store.db`, applying the schema.
    ///
    /// # Errors
    /// Returns [`DbError`] if the database file cannot be opened or the schema fails.
    pub async fn open() -> Result<Self, DbError> {
        let path = db_path("fs-store.db");
