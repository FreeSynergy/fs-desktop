...
pub async fn refresh_lens(lens: &Lens) -> Vec<LensItem> {
    // Attempt to query the running bus via HTTP
    if let Ok(items) = query_via_bus(&lens.query).await {
        return items;
    }

    // Fallback: return demonstration items so the UI is non-empty
    demo_items(&lens.query)
}

async fn query_via_bus(query: &str) -> Result<Vec<LensItem>, String> {
