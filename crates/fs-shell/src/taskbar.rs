...
pub fn Taskbar(apps: Vec<AppEntry>, on_launch: EventHandler<String>) -> Element {
    rsx! {
        div {
            class: "fs-taskbar",
            style: "display: flex; align-items: center; height: 48px; background: var(--fs-color-bg-sidebar); padding: 0 8px; gap: 4px;",
...
fn TaskbarApp(app: AppEntry, on_click: EventHandler<MouseEvent>) -> Element {
    ...
    rsx! {
        button {
            class: if running { "fs-taskbar__app fs-taskbar__app--running" } else { "fs-taskbar__app" },
            style: "display: flex; flex-direction: column; align-items: center; background: none; border: none; cursor: pointer; padding: 4px 8px; color: var(--fs-color-text-inverse); position: relative;",
            title: "{app.label_key}",
            onclick: on_click,
            ...
            if running {
                div {
                    style: "position: absolute; bottom: 2px; width: 4px; height: 4px; border-radius: 50%; background: var(--fs-color-primary);"
                }
            }
        }
    }
...
fn SystemTray() -> Element {
    rsx! {
        div {
            class: "fs-tray",
            style: "display: flex; align-items: center; gap: 8px; padding: 0 8px; color: var(--fs-color-text-inverse); font-size: 14px;",
    ...
    rsx! {
        div {
            ...
            style: "display: flex; flex-direction: column; align-items: center; \
                    color: var(--fs-color-text-inverse, #e2e8f0); padding: 0 12px; min-width: 72px;",
            span {
                style: "font-size: 13px; font-weight: 600; line-height: 1.2;",
                "{time_str}"
            }
            span {
                style: "font-size: 10px; color: var(--fs-color-text-muted, #94a3b8); line-height: 1.2;",
                "{date_str}"
            }
        }
    }
...
pub fn TaskbarLauncherBtn(on_click: EventHandler<MouseEvent>) -> Element {
    rsx! {
        button {
            class: "fs-taskbar__launcher",
            style: "font-size: 20px; background: none; border: none; cursor: pointer; \
                    color: var(--fs-color-text-inverse); padding: 4px 8px;",
            title: "App Launcher",
            onclick: on_click,
            "⊞"
        }
...
pub fn TaskbarSeparator() -> Element {
    rsx! {
        div { style: "width: 1px; height: 32px; background: var(--fs-color-border-default); margin: 0 4px;" }
    }
}
