...
// K4: Bookmarks + History (in-memory, to be persisted via fsd-db in production)
...
pub fn BrowserApp() -> Element {
    ...
    rsx! {
        ...
        div {
                    ...
                    onclick: move |_| {
                        ...
                        if !url.is_empty() {
                            if let Some(bm) = add_bookmark(&url, &url) {
                                ...
                                status_msg.set(Some(fsn_i18n::t("browser.bookmarks.added")));
                            }
                        }
                    },
                    "☆"
            ...
            // ── Tab bar ───────────────────────────────────────────────────
            div {
                class: "fsd-browser__tabbar",

                for tab in tabs.read().clone().iter() {
                    div {
                        key: "{tab.id}",
                        class: if tab.id == *active_tab.read() { "fsd-browser__tab fsd-browser__tab--active" } else { "fsd-browser__tab" },
                        onclick: {
                            let tab_id = tab.id;
            ...
            div {
                ...
                div {
                    ...
                    if current_url.is_empty() {
                        ...
                        div {
                            class: "fsd-browser__newtab",
                            span { style: "font-size: 48px;", "🌐" }
                            p {
                                style: "color: var(--fsn-color-text-muted); margin-top: 12px;",
                                {fsn_i18n::t("browser.url_placeholder")}
                            }
                        }
                    } else {
...
fn BookmarksPanel(
    ...
    rsx! {
        div { class: "fsd-browser__panel-inner",
            h3 { style: "margin: 0 0 12px; font-size: 14px;", {fsn_i18n::t("browser.bookmarks")} }
            if bookmarks.is_empty() {
                p { style: "color: var(--fsn-color-text-muted); font-size: 13px;",
                    {fsn_i18n::t("browser.bookmarks.empty")}
                }
            }
            for bm in bookmarks.iter() {
                div { key: "{bm.id}",
                    class: "fsd-browser__panel-row",
                    div {
                        class: "fsd-browser__panel-row-main",
                        onclick: {
                    ...
                    button {
                        class: "fsd-browser__panel-row-action",
                        onclick: {
                            let id = bm.id;
    ...
    rsx! {
        div { class: "fsd-browser__panel-inner",
            div { style: "display: flex; align-items: center; gap: 8px; margin-bottom: 12px;",
                h3 { style: "margin: 0; flex: 1; font-size: 14px;", {fsn_i18n::t("browser.history")} }
                button {
                    class: "fsd-browser__panel-row-action",
                    onclick: move |_| on_clear.call(()),
                    {fsn_i18n::t("browser.history.clear")}
                }
            }
            if history.is_empty() {
                p { style: "color: var(--fsn-color-text-muted); font-size: 13px;",
                    {fsn_i18n::t("browser.history.empty")}
                }
            }
...
fn DownloadsPanel(downloads: Vec<DownloadEntry>) -> Element {
    rsx! {
        div { class: "fsd-browser__panel-inner",
            h3 { style: "margin: 0 0 12px; font-size: 14px;", {fsn_i18n::t("browser.downloads")} }
            if downloads.is_empty() {
                p { style: "color: var(--fsn-color-text-muted); font-size: 13px;",
                    {fsn_i18n::t("browser.downloads.empty")}
                }
            }
            for dl in downloads.iter() {
                div { key: "{dl.id}",
                    class: "fsd-browser__panel-row",
                    div { class: "fsd-browser__panel-row-main",
                    ...
                    span {
                        style: "font-size: 12px; color: var(--fsn-color-text-muted); white-space: nowrap;",
                        "{dl.status.label()}"
                    }
                }
            }
...
.fsd-browser__nav-btn:hover {
    background: var(--fsn-color-bg-elevated);
}
.fsd-browser__nav-btn--active {
    color: var(--fsn-color-primary, #06b6d4);
}

.fsd-browser__address {
    flex: 1;
    background: var(--fsn-color-bg-base);
...
.fsd-browser__tab:hover {
    background: var(--fsn-color-bg-elevated);
}
.fsd-browser__tab--active {
    background: var(--fsn-color-bg-base);
    color: var(--fsn-color-text-primary);
    border-bottom: 2px solid var(--fsn-color-primary, #06b6d4);
}

.fsd-browser__tab-title {
    overflow: hidden;
    text-overflow: ellipsis;
    max-width: 110px;
}

.fsd-browser__tab-close {
    background: transparent;
    border: none;
    color: var(--fsn-color-text-muted);
    cursor: pointer;
...
.fsd-browser__tab-close:hover {
    background: rgba(239,68,68,0.2);
    color: #ef4444;
}

.fsd-browser__new-tab {
    background: transparent;
    border: none;
    color: var(--fsn-color-text-muted);
    cursor: pointer;
...
.fsd-browser__panel {
    width: 280px;
    flex-shrink: 0;
    border-left: 1px solid var(--fsn-color-border-default);
    background: var(--fsn-color-bg-surface);
    overflow-y: auto;
}

.fsd-browser__panel-inner {
    padding: 16px;
}

.fsd-browser__panel-row {
    display: flex;
    align-items: center;
    gap: 8px;
    padding: 6px 0;
...
.fsd-browser__panel-row-main:hover span:first-child {
    color: var(--fsn-color-primary, #06b6d4);
}

.fsd-browser__panel-row-action {
    background: transparent;
    border: none;
    color: var(--fsn-color-text-muted);
    cursor: pointer;
...
.fsd-browser__panel-row-action:hover {
    background: var(--fsn-color-bg-elevated);
}
"#;
