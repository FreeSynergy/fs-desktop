...
///
/// Three tabs:
///   Active:  select interface language + locale formats
...
/// Active tab in the Language settings panel.
#[derive(Clone, PartialEq, Debug)]
...
pub fn LanguageSettings() -> Element {
    ...
    let mut active_tab = use_signal(|| LangTab::Active);
    ...
    rsx! {
        div {
            ...
            div { style: "flex: 1; overflow-y: auto;",
                ...
                // ── Tab 1: Active Language + Locale Formats ──────────────────
                if *active_tab.read() == LangTab::Active {
                    div {
                ...
                // ── Tab 2: Install Languages from Store ──────────────────────
                if *active_tab.read() == LangTab::Install {
                    div { style: "padding: 24px; max-width: 560px;",
                        SectionHeading { label: fsn_i18n::t("settings.language.tabs.install") }
                        p {
                            style: "font-size: 13px; color: var(--fsn-color-text-muted); \
                                    margin: 0 0 16px 0;",
                            {fsn_i18n::t("settings.language.install_hint")}
                        }
                        AvailableLanguages {
                ...
                // ── Tab 3: Edit / Create Languages ───────────────────────────
                if *active_tab.read() == LangTab::Edit {
                    div {
                        style: "padding: 24px; max-width: 560px; \
                        ...
                        p {
                            style: "font-size: 13px; color: var(--fsn-color-text-muted); margin: 0;",
                        ...
                        div {
                            ...
                            div { style: "display: flex; flex-wrap: wrap; gap: 8px;",
                                for entry in installed.read().clone().into_iter()
                                    .filter(|e| e.code != "en")
                                {
                                    {
                                        let code = entry.code.clone();
...
#[component]
fn LangTabBar(active: LangTab, on_select: EventHandler<LangTab>) -> Element {
    rsx! {
        div {
            style: "display: flex; border-bottom: 1px solid var(--fsn-color-border-default); \
                    ...
                    background: var(--fsn-color-bg-surface);",
            TabBtn {
                label:     fsn_i18n::t("settings.language.tabs.active"),
                is_active: active == LangTab::Active,
                onclick:   move |_| on_select.call(LangTab::Active),
            }
            TabBtn {
                label:     fsn_i18n::t("settings.language.tabs.install"),
                is_active: active == LangTab::Install,
                onclick:   move |_| on_select.call(LangTab::Install),
            }
            TabBtn {
                label:     fsn_i18n::t("settings.language.tabs.edit"),
                is_active: active == LangTab::Edit,
                onclick:   move |_| on_select.call(LangTab::Edit),
            }
        }
    }
}
    ...
    rsx! {
        button {
            style: format!(
                ...
                if is_active { "var(--fsn-color-primary)" } else { "var(--fsn-color-text-secondary, var(--fsn-color-text-muted))" },
                if is_active { "var(--fsn-color-primary)" } else { "transparent" },
            ),
            onclick,
            "{label}"
        }
